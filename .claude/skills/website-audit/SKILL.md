...
Use the `AskUserQuestion` tool for quick structured questions where appropriate, but keep it conversational. If context is already available from the conversation (e.g. the user already shared the URL and described their product), skip those questions and confirm your understanding instead.
...
If no live URL is given, run the local site: `pnpm dev` and crawl `http://localhost:5173`, using `src/routes.ts` as the route list. Prefer reading the source of truth in `src/data/content.ts` alongside the rendered pages — corrections land there, not in the HTML.

Work through `references/checklist.md` for the detailed per-page checks.
...
Always produce the audit as a .md file saved to `.claude/artifacts/`. Name it `[site-name]-website-audit.md`.
...
When presenting the audit, give a brief conversational summary of the top findings, then surface the file with `SendUserFile`. Don't paste the entire audit into the chat.
