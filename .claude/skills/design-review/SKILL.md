...
Use whichever is available, in order: Chrome MCP (`mcp__claude-in-chrome__*`), Playwright MCP, `playwright-cli`. You need screenshots at desktop (1440px), tablet (768px), and mobile (390px) widths, in both light and dark mode.

## URL Resolution

Prefer a deployed/live URL if the user gives one. Otherwise start the local dev server (`pnpm dev`) and review `http://localhost:5173`. Review every route in `src/routes.ts`, not just the home page.
...
Write findings to `.claude/artifacts/design-review.md`:
